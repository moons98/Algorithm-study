...
def copy_rabbit(rabbit):
    return Rabbit(rabbit.pid, rabbit.x, rabbit.y, rabbit.j)
...
def race(elem):
    ...
    for _ in range(k):
        ...
        nxt_rabbit = copy_rabbit(cur_rabbit)
        nxt_rabbit.x = 0
        nxt_rabbit.y = 0

        # 위로 이동
        up_rabbit = get_up_rabbit(copy_rabbit(cur_rabbit), d)
        if compare(nxt_rabbit, up_rabbit):
            nxt_rabbit = up_rabbit

        # 아래로 이동
        down_rabbit = get_down_rabbit(copy_rabbit(cur_rabbit), d)
        if compare(nxt_rabbit, down_rabbit):
            nxt_rabbit = down_rabbit

        # 왼쪽으로 이동
        left_rabbit = get_left_rabbit(copy_rabbit(cur_rabbit), d)
        if compare(nxt_rabbit, left_rabbit):
            nxt_rabbit = left_rabbit

        # 오른쪽으로 이동
        right_rabbit = get_right_rabbit(copy_rabbit(cur_rabbit), d)
...
for _ in range(q):
    ...
    # print("rabbit: ")
    # for i in range(p):
    #     print("loc:", loc[i], "pid:", pids[i], "jump_cnt:", jump_cnt[i])
